...
# inputs
df = pd.read_csv("pfam_results_AZ20.csv", index_col='Unnamed: 0')
md = pd.read_csv("AZ20_metadata.csv", index_col="sample_name")
input_dir = 'out_AZ20/msa_files/'
col_names = ['PF_1', 'PF_2', 'n_samples', 'n_groups', 'pseudo_f', 'p_val']
base_df = pd.DataFrame(columns=col_names)

# identify top 100 pfams
top100_df = df.loc[df["n_samples"] > 50].sort_values(by="pseudo_f", ascending=False)
...
for combo in itertools.combinations(top100_df.index, 2):
	...
	base_df.to_csv('meta_top_results_AZ20.csv')
